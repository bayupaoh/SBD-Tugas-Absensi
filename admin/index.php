...
<?php
		...
		$login=mysql_query("select * from guru where email='$user_email' and password='$user_password' and status='Admin'");
		...
		if ($cek_login != 1)
		 {
			...
			while ($row=mysql_fetch_array($login))
			{
				...
				$_SESSION['user_name']=$row["nama_guru"];
    ...
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
      <header class="mdl-layout__header">
        <div class="mdl-layout__header-row">
          <!-- Title -->
          <span class="mdl-layout-title">Resto Broto</span>
        </div>
        <!-- Tabs -->
      </header>
